...
export default {
  ...
  methods: {
    ...
    async handleETaxInvoice() {
      const eTaxInvoiceUrl = `${this.config.public.eTaxInvoiceUrl}/#/public/request-tax-invoice?sellerId=${this.urlCurrent.sellerId}&token=${this.urlCurrent.token}`
      window.location.replace(eTaxInvoiceUrl)
    }
  }
}
</script>
    ...
    <div class="bg-white w-full min-h-screen md:w-[500px] md:rounded-lg shadow-lg">
      <div
        ...
        <div class="flex flex-col w-full h-[100vh] justify-center items-center text-center p-2">
          <p class="text-sm	text-stone-800 mb-2">กำลังไปยังหน้าสำหรับขอ E-RECEIPT & E-TAX INVOICE</p>
          <p class="text-sm	text-stone-800">REDIRECTING TO REQUEST E-RECEIPT & E-TAX INVOICE.</p>
        </div>
      </div>
    </div>
  </div>
